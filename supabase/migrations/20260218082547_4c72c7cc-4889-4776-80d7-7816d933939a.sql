...
-- Update claim_deal to auto-expire the deal when the last unique code is assigned
...
BEGIN
  ...
  IF v_deal.discount_type = 'unique' THEN
    ...
    -- Check if there are remaining available codes
    SELECT COUNT(*) INTO v_remaining
    FROM unique_codes
    WHERE deal_id = p_deal_id AND status = 'available';

    -- If no more codes available, auto-expire the deal
